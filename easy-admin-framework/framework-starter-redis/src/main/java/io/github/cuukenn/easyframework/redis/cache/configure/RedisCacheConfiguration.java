	...
	public CacheErrorHandler errorHandler() {
		...
		return new CacheErrorHandler() {
			...
			public void handleCacheGetError(RuntimeException exception, Cache cache, Object key) {
				logger.error("[spring cache][handleCacheGetError][cache:[{}],key:[{}]]", cache, key, exception);
			}

			@Override
			public void handleCachePutError(RuntimeException exception, Cache cache, Object key, Object value) {
				logger.error("[spring cache][handleCachePutError][cache:[{}],key:[{}]]", cache, key, exception);
			}

			@Override
			public void handleCacheEvictError(RuntimeException exception, Cache cache, Object key) {
				logger.error("[spring cache][handleCacheEvictError][cache:[{}],key:[{}]]", cache, key, exception);
			}

			@Override
			public void handleCacheClearError(RuntimeException exception, Cache cache) {
				logger.error("[spring cache][handleCacheClearError][cache:[{}]]", cache, exception);
			}
		};
	}
}
